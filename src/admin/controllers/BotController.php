...
class BotController extends Controller {
	...
	public function actions() {
		...
		$actions['index']['query'] = Query::forge(ClientHelper::getQueryFromRequest())/*->with('classes')*/
		;
	...
	public function actionTest($bot_id) {
		...
		$splitCollection = SplitHelper::split($botEntity->trains, $trainPercent);
		...
		$result = [
			'total_all' => count($botEntity->trains),
			'total_train' => count($splitCollection->train),
			'total_test' => count($splitCollection->test),
			'total_error' => $errors,
			'total_success' => count($splitCollection->test) - $errors,
